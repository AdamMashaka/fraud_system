...
def analysis(request,id):
    ...
    # Data Shape
    data_shape = df.shape
   #lets check if "class" exist in our data set  
      
    # Unique Target Values
    # Assuming 'Class' is your target column
# Unique Target Values
   # Unique Target Values
    columns_to_check = ['online_order','class','distance_from_home']
    columns_to_check = [column for column in columns_to_check if column in df.columns]
    unique_values = {column: df[column].unique().tolist() for column in columns_to_check}
    try:
          unique_values = {column: df[column].unique().tolist() for column in columns_to_check}
      ...
      #check if 'fraud' collumn exist in our data set
    if 'fraud' in df.columns:
        unique_targets = unique_values['online_order']
    else:
        unique_targets = []
    # Percentages
    
    percent_no_problem = 100 * (df[df['online_order'] == 0].shape[0] / df.shape[0])
    percent_problem = 100 * (df[df['online_order'] == 1].shape[0] / df.shape[0])

    # Null values check
    has_null = df.isnull().any().any()

    # Transactions
    fraud_transactions = df[df['online_order'] == 1]
    normal_transactions = df[df['online_order'] == 0]
